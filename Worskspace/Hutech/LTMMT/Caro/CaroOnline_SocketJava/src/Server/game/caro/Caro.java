...
public class Caro extends GameLogic {
    ...
    public void addHistory(int row, int col, String playeruserName) {
        History newHis = new History(row, col, playeruserName);
        history.add(newHis);
        preMove = newHis;
    }
    ...
    public boolean move(int row, int col, String playeruserName) {
        // nếu người này đã đánh trước đó thì không cho đánh nữa
        if (preMove != null && preMove.getPlayeruserName().equals(playeruserName)) {
            return false;
        }
        ...
        board[row][col] = playeruserName;
        addHistory(row, col, playeruserName);
        return true;
    }
    ...
    public String getLastMoveuserName() {
        return history.get(history.size() - 1).getPlayeruserName();
    }
